...
Formatter::start (const string &file, string *errmsg)
{
  ...
  {
    // Builds a command line command such like:
    //                       "./ncl_generator[.lua]\
    //                        padding.json\
    //                        template.j2\
    //                       -d"; (debug mode only)

    string _file = g_path_get_basename (file.c_str ());
    string _template = g_path_get_basename(_opts.templates.c_str ());

    if(xstrhassuffix (_template, ".j2"))
    {
    ...
    else if(xstrhassuffix (_template, ".mustache"))
    {
      // Iterate over /templates subfolder to populate an array of templates unless in the case
      // the file read matches _template.c_str()  

      GError *error;
      char* _pwd = g_get_current_dir();
      string templatesDir = xpathbuild(_pwd, "templates");
      GDir* dir =  g_dir_open(templatesDir.c_str(), 0, &error);
      const char* filename;
      GPtrArray* template_names = g_ptr_array_new();

      while ((filename = g_dir_read_name(dir)))
      {
        TRACE("content in dir: %s", filename);
        if(g_strcmp0(filename, _template.c_str ()) != 0)
        {
          g_ptr_array_add(template_names, gpointer(filename));
        }
      }
      ...
      g_free(command);
      g_free(_pwd);
      g_ptr_array_free(template_names,TRUE);
    }


    // get file generated by script
    char* _pwd = g_get_current_dir();
    string data;
    GError *error;
    GDir* dir =  g_dir_open(_pwd, 0, &error);
    const char* filename;
    int max_score = 0;
    string nclFile;
    ...
    {
      TRACE("content in dir: %s", filename);
      if(g_str_has_suffix(filename, ".ncl")){
          int score = xstrmatchcmp(filename, _template);
          if (score > max_score){
            max_score = score;
            nclFile = filename;
          }
    ...
    TRACE("ncl file: %s", nclFile.c_str());
    g_dir_close(dir);

    string nclFilePath = xpathmakeabs(nclFile);

    if (_doc == nullptr)
      _doc = Parser::parseFile (nclFilePath, w, h, errmsg);
    if (unlikely (_doc == nullptr))
      return false;
  ...
  if (_opts.debug)
    {
      ...
      Rect ink;
      info = xstrbuild ("%s %s: #%lu %" GINGA_TIME_FORMAT " %.1ffps",
                        _docPath.c_str (), _opts.templates.c_str (), _lastTickFrameNo,
                              ...
                              string value)
{
  ...
  bool suffix;
  if (xstrhassuffix(value, ".j2"))
      suffix = true;
  else {
    if (xstrhassuffix(value, ".mustache"))
      suffix = true;
  }
  if (value != "" && !suffix)
  {
